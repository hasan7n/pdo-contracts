from pdo.download.plugins.download_token import (
    cmd_mint_tokens,
    cmd_do_download,
...
def _setup_register_issuer_context(state, user, issuer_contract_id):
    ...
    context_file = os.path.join(IDENTITY_CONTEXT_FILES, "policy_agent.toml")
    Context.LoadContextFile(state, bindings, context_file)
    issuer_context_path = f"identity.{label}.policy_agent"
    state.set(
...
def _invoke_download_token(state, contract_id, user, url, cmd_class, **cmd_args):
...
def create_download_token(state, user, url):

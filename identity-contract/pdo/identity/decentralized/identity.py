...
def _setup_context(state, user):
    ...
    context_file = os.path.join(CONTEXT_FILES, "identity.toml")
    Context.LoadContextFile(state, bindings, context_file)
    context = Context(state, prefix=f"identity.{label}.wallet")
    return context


def _invoke_identity(state, contract_id, user, cmd_class, **cmd_args):
    # setup context
...
def sign(state, contract_id, user, **cmd_args):
    return _invoke_identity(state, contract_id, user, cmd_sign, **cmd_args)


def verify(state, contract_id, user, **cmd_args):
    return _invoke_identity(state, contract_id, user, cmd_verify, **cmd_args)


def add_vc(state, contract_id, user, **cmd_args):
    return _invoke_identity(state, contract_id, user, cmd_add_vc, **cmd_args)


def get_vc_list(state, contract_id, user, **cmd_args):
    return _invoke_identity(state, contract_id, user, cmd_get_vc_list, **cmd_args)


def get_vp(state, contract_id, user, **cmd_args):
    return _invoke_identity(state, contract_id, user, cmd_get_vp, **cmd_args)


def create_identity(state, user, **cmd_args):
    # setup context
    context = _setup_context(state, user)

    # run
    save_file = pcommand.invoke_contract_cmd(
        cmd_create_identity, state, context, **cmd_args
    )

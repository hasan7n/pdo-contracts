...
# Programmatic (non-shell) API for the external key authority, mirroring the
# identity family's decentralized/signature_authority.py.

import os
import random
import string

from pdo.authority.plugins.external_key_authority import (
...
def _setup_context(state, user):
    ...
    context_file = os.path.join(CONTEXT_FILES, "external_key_authority.toml")
...
def create_external_key_authority(state, user, **cmd_args):
    context = _setup_context(state, user)
    save_file = pcommand.invoke_contract_cmd(
        cmd_create_external_key_authority, state, context, **cmd_args
    )
